...
public class CustomUserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username)
        ...
        Optional <ClubMember> result = clubMemberRepository.getWithRoles(username);//getWithRoles 클럽멤버에 있는 유저 네임을 찾는다.
        if(result.isEmpty()){
            throw new UsernameNotFoundException("없는 사용자 이름");
        }

        //존재하는 사용자 찾아오기
        ClubMember member = result.get(); // 비밀번호확인 이없음
        ClubMemberSecurityDTO clubMemberSecurityDTO = new ClubMemberSecurityDTO(member.getMid(),
                //ClubMemberSecurityDTO가 비밀번호 일치여부 수행하며, User를 상속받았을때 해당 super()에서 패스워드 확인
                member.getMpw(), member.getEmail(), member.getEmail(), member.isDel(),
                false ,
